<tr>
    <td>
        @if($aplicacao->situacao == "Aberta" || $aplicacao->situacao == 'Pendente')
            <input {{ isset($visualizar) ? 'disabled' : '' }} class="form-check-input" data-medicamento="{{ $aplicacao->medicamento->unidade }}" data-nome-medicamento="{{ $aplicacao->medicamento->nome }}" data-medicamento-id="{{ $aplicacao->medicamento->id }}" data-quantidade="{{ $aplicacao->quantidade }}" type="checkbox" value="Sim" onclick="controle_pendente({{ $aplicacao->medicamento->id }}, this)" name="controle_pendente_{{ $aplicacao->medicamento->id }}" id="controle_pendente_{{ $aplicacao->medicamento->id }}"></td>
        @endif
