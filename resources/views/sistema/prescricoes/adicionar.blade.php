...
<div class="card card-border-shadow-primary mb-4">
    <div class="card-body">
        <div class="d-flex justify-content-between">
            <h4 class="card-title">Adicionar Prescrição</h4>
        ...
        <form id="form_cadastro_prescricao" method="post" enctype="multipart/form-data" action="{{ route('sistema.prescricoes.insert') }}">
            @csrf
            ...
            <div class="row mt-2 gy-4 align-items-end">
                ...
                <div class="col-md-4">
                    <div class="form-floating form-floating-outline">
                        <input class="form-control" type="text" id="valor_tratamento" name="valor_tratamento" onkeypress="return(MascaraMoeda(this,'.',',',event))" onkeyup="recalcular_financeiro()"/>
                        <label for="valor_tratamento">Valor do Tratamento (R$):</label>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-floating form-floating-outline">
                        <input class="form-control" type="text" id="credito_em_aberto" name="credito_em_aberto" onkeypress="return(MascaraMoeda(this,'.',',',event))" onkeyup="recalcular_financeiro()"/>
                        <label for="credito_em_aberto">Crédito em Aberto (R$):</label>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-floating form-floating-outline">
                        <input class="form-control" type="file" multiple id="anexos" name="anexos[]" onchange="atualizar_aviso_anexo()"/>
                        <label for="anexos">Anexos (Prescrição Médica):</label>
                    </div>
                    <small class="text-danger fw-bold" id="aviso_anexo" style="display:none;">
                        <i class="mdi mdi-alert-circle-outline me-1"></i>É obrigatório anexar a prescrição médica quando alguma semana tiver medicamento em Ampola ou Miligrama.
                    </small>
                </div>
                <div class="col-md-12">
                    <div class="form-floating form-floating-outline">
            ...
            <div class="card card-border-shadow-primary mt-4">
                <div class="card-body">
                    ...
                    <hr>
                    <p class="text-muted mb-2">As parcelas são geradas a partir do valor do tratamento, uma por semana <b>com medicação</b> (semanas de pausa ficam fora do cálculo).</p>
                    <div class="table-responsive">
                        <table class="table table-sm">
                            <thead class="table-light">
                                <tr>
                            ...
                            <tbody id="tabela_parcelas">
                                <tr><td colspan="4" class="text-center text-muted">Informe o valor do tratamento e as semanas com medicação para gerar as parcelas.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-primary d-flex align-items-center">
                    <span class="mdi mdi-checkbox-marked-circle-outline mdi-24px me-2"></span>Confirmar Cadastro de Prescrição
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p class="fw-semibold mb-3">Revise o resumo abaixo antes de confirmar o cadastro:</p>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-sm">
                        ...
                        <tfoot class="table-light">
                            <tr>
                                <th colspan="3" class="text-end fw-bold">Valor do Tratamento:</th>
                                <th id="confirmacao_valor_tratamento" class="text-end fw-bold font-monospace">R$ 0,00</th>
                            </tr>
                            <tr id="row_confirmacao_credito" style="display:none;">
                                <th colspan="3" class="text-end fw-bold">(-) Crédito em Aberto:</th>
                                <th id="confirmacao_credito" class="text-end fw-bold font-monospace">R$ 0,00</th>
                            </tr>
                            <tr>
                                <th colspan="3" class="text-end fw-bold text-primary">Total a Parcelar:</th>
...
let medicamentosMap = {
    @foreach($medicamentos as $medicamento)
        "{{ $medicamento->id }}": { unidade: "{{ $medicamento->unidade }}" },
    @endforeach
};
let modalGerador;
...
function adicionar_procedimento(dt = '', obs = '', pausa = false){
    ...
    div.innerHTML = `
        <div class="card-body">
            <div class="d-flex justify-content-between">
                <h5 class="card-title">Semana ${contador}</h5>
...
document.getElementById('botao_gerador').addEventListener('click', ()=>{
    // zera o gerador a cada abertura
...
function aplicar_medicamentos_na_semana(n, meds){
    ...
    meds.forEach(m => {
        if(existentes.has(String(m.mid))) return; // já está na semana
        if(primeiraVazia){
...
function garantir_sequencia_semanal(){
    // preenche os "buracos" semanais com semanas de pausa (sem medicação)
    let datas = [];
    let cont = parseInt(document.getElementById('contador_procedimentos').value);
    for(let n = 1; n <= cont; n++){
        let card = document.getElementById('card_' + n);
...
function ordenar_semanas(){
    // ordena os cards por data e renumera as semanas
    ...
    cards.forEach((c, idx) => {
        ...
        if(h) h.textContent = 'Semana ' + (idx+1);
    });
}

function remover_semanas_vazias(){
    // remove semanas completamente vazias (sem data, sem pausa e sem medicação)
    let cont = parseInt(document.getElementById('contador_procedimentos').value);
    for(let n = 1; n <= cont; n++){
        let card = document.getElementById('card_' + n);
        if(!card) continue;
...
function gera_procedimentos_gerador(){
    ...
    // se a semana 1 está vazia, o gerador começa por ela
    let usar_semana_1 = semana_esta_vazia(1);

    // 1) cria ou preenche as semanas de aplicação
    appDates.forEach((dt, idx) => {
        if(idx === 0 && usar_semana_1){
            // preenche a semana 1 vazia em vez de criar outra
        ...
        if(n){
            // já existe: vira aplicação e adiciona os medicamentos do gerador
    ...
    // 2) garante a sequência semanal (semanas de pausa nos intervalos)
    garantir_sequencia_semanal();

    // 3) remove semanas completamente vazias (sem data, sem pausa e sem medicação)
    remover_semanas_vazias();

    // 4) ordena por data e renumera
    ordenar_semanas();
...
// ---------- ANEXO OBRIGATÓRIO (Ampola/Miligrama) ----------
function tem_medicamento_ampola_miligrama(){
    let cont = parseInt(document.getElementById('contador_procedimentos').value);
    for(let n = 1; n <= cont; n++){
        let card = document.getElementById('card_' + n);
        if(!card) continue;
        let pausa = document.getElementById('pausa_' + n) ? document.getElementById('pausa_' + n).checked : false;
        if(pausa) continue; // semana de pausa não tem aplicação
        let rows = document.querySelectorAll('#tabela_medicamentos_' + n + ' tr select');
        for(let i = 0; i < rows.length; i++){
            let sel = rows[i];
            if(!sel.value) continue;
            let info = medicamentosMap[sel.value];
            if(info && (info.unidade === 'Ampola' || info.unidade === 'Miligrama')){
                return true;
            }
        }
    }
...
function atualizar_aviso_anexo(){
    let aviso = document.getElementById('aviso_anexo');
    if(!aviso) return true;
    let anexos = document.getElementById('anexos');
    let temAnexo = anexos && anexos.files && anexos.files.length > 0;
    let precisa = tem_medicamento_ampola_miligrama() && !temAnexo;
    aviso.style.display = precisa ? 'block' : 'none';
    return !precisa;
}
...
function recalcular_resumo_medicamentos(){
    let cont = parseInt(document.getElementById('contador_procedimentos').value);
    let mapa = new Map(); // medicamento_id -> { nome, qtd }
...
function montar_financeiro_html(){
    let valor = valor_form_db(document.getElementById('valor_tratamento').value || '0');
    let credito = valor_form_db(document.getElementById('credito_em_aberto') ? document.getElementById('credito_em_aberto').value : '0');
    let valor_parcelar = Math.max(0, valor - credito);
    let cont = parseInt(document.getElementById('contador_procedimentos').value);

    // coleta todas as semanas com data (id, dt, pausa, tem_med)
    let todas = [];
    for(let n = 1; n <= cont; n++){
        let card = document.getElementById('card_' + n);
        if(!card) continue;
    ...
    // ordena por data e define o rótulo "Semana X" (igual ao ordenar_semanas)
    todas.sort((a,b) => a.dt < b.dt ? -1 : (a.dt > b.dt ? 1 : a.n - b.n));
    todas.forEach((s, idx) => { s.rotulo = idx + 1; });

    // semanas com aplicação (para parcelas), já em ordem cronológica
    let semanas = todas.filter(s => !s.pausa && s.tem_med);
    let total = semanas.length;

    if(total === 0 || valor_parcelar <= 0){
        return {
            html: `<tr><td colspan="4" class="text-center text-muted">Informe o valor do tratamento e as semanas com medicação para gerar as parcelas.</td></tr>`,
            total: 0,
            valor: valor,
            credito: credito
        };
    ...
    semanas.forEach((s, idx) => {
        let v = base;
        if(idx === total - 1){ v = Math.round((base + resto) * 100) / 100; }
        let venc = s.dt ? s.dt.split('-').reverse().join('/') : '-';
        html += `<tr>
            <td>${idx + 1}</td>
            <td>Semana ${s.rotulo}</td>
            <td>${venc}</td>
            <td>R$ ${v.toFixed(2).replace('.', ',')}</td>
        </tr>`;
    });
    return { html, total: valor_parcelar, valor: valor, credito: credito };
}

function recalcular_financeiro(){
    recalcular_resumo_medicamentos();
    atualizar_aviso_anexo();
...
function valida_e_confirma_prescricao(){
    ...
    if(pacienteEl && !pacienteEl.value){ erros.push('Por favor, escolha o Paciente.'); }
    const medicoEl = document.getElementById('medico');
    if(medicoEl && !medicoEl.value){ erros.push('Por favor, escolha o Médico.'); }
    const tipoEl = document.getElementById('tipo_atendimento');
    if(tipoEl && !tipoEl.value){ erros.push('Por favor, selecione o Tipo de Tratamento.'); }
    const agendEl = document.getElementById('agendamento');
    if(agendEl && !agendEl.value.trim()){ erros.push('Por favor, preencha o Agendamento.'); }
    ...
    for(let i = 1; i <= cont; i++){
        ...
        if(!dt){
            ...
            if(dataProc < hoje){
                erros.push(`Semana ${i}: A Data Prevista (${formata_data_pt(dt)}) está no passado. Não é permitido cadastrar prescrição com data retroativa.`);
            }
        }
        ...
        if(pausa){
            ...
            <tr>
                ...
                <td>${dt ? formata_data_pt(dt) : ''}</td>
                <td class="text-muted"><span class="mdi mdi-minus-circle-outline me-1"></span>Pausa (sem medicação)</td>
            </tr>`;
            continue;
        }
        if(!temMed){
            erros.push(`Semana ${i}: Adicione pelo menos um medicamento ou marque "Pausa (sem medicação)".`);
            continue;
        }
        resumoSemanasHtml += `
        <tr>
    ...
    // 3. anexo obrigatório (Ampola/Miligrama)
    if(tem_medicamento_ampola_miligrama()){
        let anexos = document.getElementById('anexos');
        if(!anexos || !anexos.files || anexos.files.length === 0){
            erros.push('É obrigatório inserir o anexo (prescrição médica) pois a prescrição contém medicamentos em Ampola ou Miligrama.');
        }
    }
    ...
    document.getElementById('confirmacao_valor_tratamento').innerText = 'R$ ' + (fin.valor ? fin.valor.toFixed(2).replace('.', ',') : '0,00');
    let rowCredito = document.getElementById('row_confirmacao_credito');
    if(rowCredito){
        if(fin.credito > 0){
            rowCredito.style.display = '';
            document.getElementById('confirmacao_credito').innerText = 'R$ ' + fin.credito.toFixed(2).replace('.', ',');
        } else {
    ...
    document.getElementById('confirmar_e_salvar_prescricao').onclick = function(){
        ...
        document.getElementById('form_cadastro_prescricao').submit();
    };
}
</script>
...
window.addEventListener('load', function(){
    let formCadastro = document.getElementById('form_cadastro_prescricao');
    if(formCadastro){
